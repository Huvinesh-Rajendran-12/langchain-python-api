...
llm = ChatAnthropic(
    model="claude-3-opus-20240229", temperature=0, api_key=anthropic_api_key
)
    ...
    return {
        "messages": [
            AIMessage(
                ...
                tool_calls=[
                    {"name": "sql_db_list_tables", "args": {}, "id": "tool_list_tables"}
                ],
            )
        ]
    }
...
def process_user_query(user_query: str) -> str:
    messages = app.invoke(
        {"messages": [HumanMessage(content=user_query)]}, {"recursion_limit": 100}
    )
