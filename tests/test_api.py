...
def test_query_endpoint_success():
    ...
    with patch("src.api.main.sql_agent") as mock_sql_agent:
        ...
        mock_sql_agent.get_final_answer = AsyncMock(
            return_value="This is the final answer"
        )
...
def test_query_endpoint_error():
    ...
    with patch("src.api.main.sql_agent") as mock_sql_agent:
        ...
        assert (
            json.loads(content[1])["message"]
            == "No updates received from the SQL agent"
        )
...
async def test_process_query_with_updates():
    ...
    with patch("src.api.main.sql_agent") as mock_sql_agent:
        ...
        updates = [
            update async for update in process_query_with_updates("Test question")
        ]
